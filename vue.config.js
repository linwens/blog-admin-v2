...
module.exports = {
  ...
  devServer: {
    port: port,
    open: true, // 是否在服务启动后自动打开浏览器，可以设置启动的浏览器，如：'Google Chrome'
    overlay: { // 是否在窗口全屏展示异常信息，这里设置警告不展示，展示错误
      warnings: false,
      errors: true
    },
    proxy: {
    ...
    after: require('./mock/mock-server.js') // 所有中间件执行完以后执行自定义的中间件(联调的时候要把这里注释掉，并且修改proxy的target)
  },
  configureWebpack: {
